...
export class AgentConfig implements AgentState {
    ...
    // 更新 model
    updateModel(partial: Partial<AgentModel>) {
        this.model = {
            ...this.model,
            ...partial
        };
    ...
    updateSystemPrompt(partial: Partial<SystemPrompt>) {
        this.systemPrompt = {
            ...this.systemPrompt,
            ...partial
        };
    ...
    private formatProcedure(title: string, steps: string[]): string {
        if (!steps || steps.length === 0) return "";
        return `**${title}：**\n` +
            steps.map((step, index) => `${index + 1}. ${step}`).join("\n");
    }

    private formatInline(title: string, items: string[]): string {
